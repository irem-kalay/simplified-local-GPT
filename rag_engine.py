...
# In-memory response cache: key = lowercase query string, value = result dict
...
def initialize_rag_engine():
    ...
    try:
        ...
        # YENİ OLLAMA API UYUMU (Bug 4 Fix)
...
def retrieve_context(query: str, top_k: int = RETRIEVAL_TOP_K, query_type: str = None) -> Tuple[str, List[Dict]]:
    ...
    # Step 4b: Re-rank chunks by keyword overlap with query
    query_words = set(query.lower().split())
    ranked = sorted(
        zip(documents, metadatas),
        key=lambda pair: sum(1 for w in query_words if w in pair[0].lower()),
        reverse=True,
    )
    documents = [pair[0] for pair in ranked]
    metadatas = [pair[1] for pair in ranked]
...
def prepare_context(
    standalone_query: str,  # (Bug 2 Fix) Artık doğrudan çözümlenmiş soruyu alıyor
    chat_history: list = None,
) -> Tuple[str, str, str, List[Dict], str, Optional[str]]:
    """
    Run classify + retrieve steps only, without generation.
    Used by the streaming UI to get metadata before streaming tokens.

    Returns:
        Tuple of (standalone_query, query_type, context_text, sources, history_text, error)
    """
    if not _initialized:
        raise RuntimeError("RAG engine not initialized. Call initialize_rag_engine() first.")

    try:
        # SİLİNDİ: standalone_query = condense_query(...) (Bug 1 Fix)

        t0 = time.time()
        query_type = classify_query(standalone_query)
        print(f"⏱ classify_query: {time.time() - t0:.2f}s")

        history_text = format_chat_history(chat_history)

        t0 = time.time()
        context_text, sources = retrieve_context(standalone_query, query_type=query_type)
        print(f"⏱ retrieve_context: {time.time() - t0:.2f}s")

        return standalone_query, query_type, context_text, sources, history_text, None

    except Exception as e:
        return standalone_query, "mixed", "", [], "", str(e)
...
def generate_answer_stream(
    ...
    """
    Stream answer tokens from Ollama one by one.
    Designed to be passed directly to st.write_stream() in Streamlit.
    ...
    if not context or context == "No relevant information found in the knowledge base.":
        yield "I don't know. This question is about information not in my knowledge base."
        return

    # Build prompt (identical to generate_answer)
    base_prompt = """You are a factual assistant answering questions about famous people and places.
CRITICAL RULES:
1. Answer using ONLY facts explicitly stated in the provided context. Never use outside knowledge.
2. Each [SOURCE: X] block belongs exclusively to entity X. Never apply a fact from one
...
5. Use the conversation history ONLY to resolve pronouns (he, she, it, they) — not as a
   source of facts."""
    ...
    if history_text:
        history_section = (
            f"Previous conversation (for context only, do not answer based on this):\n"
            f"{history_text}\n\n"
        )
    ...
    user_prompt = (
        ...
        f"Question: {query}\n\n"
        f"Answer based ONLY on the context above. Use the previous conversation only to "
        f"understand what pronouns like \"he/she/it/they\" refer to. "
        f"If the information is not in the context, respond with: I don't know"
    )
    ...
    try:
        ...
        print(f"⏱ generate_answer_stream: {time.time() - t0:.2f}s")
...
def answer_question(
    ...
    try:
        ...
        print(f"⏱ generate_answer: {time.time() - t0:.2f}s")
        print(f"⏱ total pipeline: {time.time() - pipeline_start:.2f}s")
...
def clear_cache() -> None:
    ...
    All subsequent queries will be processed fresh.
    """
